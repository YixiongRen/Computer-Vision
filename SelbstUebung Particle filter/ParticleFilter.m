...
%粒子滤波初始 N 个粒子
for i = 1 : N
    p_xpart(i) = p_x_estimate + sqrt(pf_P) * randn;  %均值为1方差为5的正态分布随机数
end
...
for k = 1 : tf          %循环tf次，等同于tf次输入。k影响u的值
    %% 系统本身的真实值与观测值
    % 模拟系统 linear ==0.5  状态值（本质方程）x = Ax +Bu +Q     观测值(表象）y = Hx +R
    ...
    for i = 1 : N           %计算每个粒子当前的权值
        ...
        p_ypart = p_xpartminus(i)^2 / 20; %粒子的观测值 
        
        
        p_vhat = y - p_ypart;% 观测和粒子观测的差 
        p_q(i) = (1 / sqrt(R) / sqrt(2*pi)) * exp(-p_vhat^2 / 2 / R); % 各个粒子的权值。p_q(i)=P(y|Xi)就是在每个粒子
                                                                      % 附近取到y的概率。所以单独考虑每个粒子，
                                                                      % 都有各自的正态分布。均值μ为粒子观测值，
    end                                                               % 方差为观测误差R。观测值y就是正态分布函数里的自变量x
                                                                      % 因为y与Xi初始值差别很大，所以大多数粒子的权值都很小。
   ...
   % 重采样 权重大的粒子多采点，权重小的粒子少采点, 权值太小不采点。
    for i = 1 : N 
        p_u = rand; %均值0.5   幅度0~1之间
        p_qtempsum = 0; 
        for j = 1 : N 
            p_qtempsum = p_qtempsum + p_q(j); 
            if p_qtempsum >= p_u              %每次权值叠加超过0~1之间的随机数rand，就进入if语句
                p_xpart(i) = p_xpartminus(j); %将最后越过rand的那个粒子复制进下一轮，作为先验值
                break; 
            end 
        end 
    end 
    ...
    p_x_estimate = mean(p_xpart); %粒子位置的算数均值
%     p_x_estimate = 0;
%     for i = 1 : N
%         p_x_estimate =p_x_estimate + p_q(i)*p_xpart(i);%粒子位置的加权均值
%     end
...
%另外存在y_2obser(i) 中；
 ...
 %% 进行画图程序 
    x_array = [x_array,x];          %系统真实值
    e_x_estimate_array = [e_x_estimate_array,e_x_estimate];%扩展卡尔曼
    p_x_estimate_array = [p_x_estimate_array,p_x_estimate];%粒子滤波
    u_x_estimate_array = [u_x_estimate_array,u_x_estimate];%不敏卡尔曼
